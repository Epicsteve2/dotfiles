...
local extra_ctrl_r="--layout=default --preview='bat --color=always --plain --plain --language zsh <<<{}' --preview-window=down:3:wrap"
    ...
    selected=( $(fc -rl 1 | perl -ne 'print if !$seen{(/^\s*[0-9]+\**\s+(.*)/, $1)}++' |
      FZF_DEFAULT_OPTS="$extra_ctrl_r --min-height=18 --height=40% -n2..,.. --tiebreak=index --header='tab to edit, enter to execute' --bind=ctrl-r:toggle-sort,ctrl-z:ignore --expect=tab --query=${(qqq)LBUFFER} +m" $(__fzfcmd)) )
    local ret=$?
    if [ -n "$selected" ]; then
      local select=0
